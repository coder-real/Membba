...
export default function MembersPage() {
  ...
  const handleRemove = async (sub) => {
    if (!window.confirm(`Remove ${sub.email} from ${sub.communities?.name}? This will kick them from the group.`)) return
    setRemoving(sub.id)
    try {
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-[13px] font-semibold capitalize transition-all ${
              filter === f
                ...
                : 'bg-[#111] border border-white/[0.08] text-white/45 hover:text-white/70 hover:border-white/15'
            }`}
          >
            {f}
            {f !== 'all' && (
              <span className="ml-2 text-[11px] opacity-60">({subscriptions.filter(s => s.status === f).length})</span>
            )}
          </button>
        ))}
      </div>
            ...
            <table className="w-full min-w-[800px]">
              <thead>
                <tr className="border-b border-white/[0.05]">
                  {['Subscriber', 'Community / Plan', 'Telegram ID', 'Started', 'Expires', 'Status', 'Action'].map(h => (
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {filtered.map(s => (
                  <tr key={s.id} className={`hover:bg-white/[0.02] transition-colors ${s.status !== 'active' ? 'opacity-50' : ''}`}>
                    <td className="px-6 py-4 text-[13.5px] text-white/75">{s.email}</td>
                    <td className="px-6 py-4">
                      <span className="text-[13.5px] text-white/80">{s.communities?.name}</span>
                      {s.plans?.name && <span className="text-white/30 ml-2 text-[12px]">· {s.plans.name}</span>}
                    </td>
                    <td className="px-6 py-4 font-mono text-[11.5px] text-white/35">{s.telegram_user_id || '—'}</td>
                    <td className="px-6 py-4 text-[12.5px] text-white/40">{new Date(s.started_at).toLocaleDateString()}</td>
                    <td className="px-6 py-4 text-[12.5px] text-white/40">{new Date(s.expires_at).toLocaleDateString()}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      {s.status === 'active' ? (
                        <button
                          ...
                          className="text-[12px] px-3.5 py-1.5 rounded-lg border border-red-500/20 text-red-400/80 hover:bg-red-500/5 hover:text-red-400 hover:border-red-500/35 disabled:opacity-40 transition-all font-medium"
                        >
